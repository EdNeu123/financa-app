...
export async function create(uid, raw, existing = [], planLimit = 30) {
  if (!uid) return { success: false, error: 'Não autenticado' };
  if (existing.length >= planLimit) return { success: false, error: `Limite de ${planLimit} metas no seu plano.${planLimit < 30 ? ' Faça upgrade para o Pro.' : ''}` };
